...
MunsellColor::~MunsellColor()
{
	...
	flow.release();
	frame.release();
	motion2color.release();
}
void MunsellColor::makecolorwheel(vector<Scalar> &colorwheel)
{
	int RY = 15;
	int YG = 6;
	int GC = 4;
...
void MunsellColor::motionToColor(Mat flow, Mat &color)
{
	if (color.empty())
